...
class BillService {
  async register({ body, user }) {
    ...
    try {
      const newBill = await prismaClient.$transaction(
        async (prismaTransaction) => {
          const { customerId, billDetails } = billRequest;

          const customer = await customerRepository.findCustomerTransactionById(
            customerId,
            prismaTransaction,
          );
          if (!customer) {
          ...
          await Promise.all(
            billDetails.map(async ({ product }) => {
              const productResult =
                await productRepository.findProductTransactionById(
                  product.id,
                  prismaTransaction,
                );
              if (!productResult) {
          ...
          await Promise.all(
            billDetails.map(async (billDetail) => {
              ...
              await billDetailRepository.createBillDetail(
                billDetailData,
                prismaTransaction,
              );
            }),
